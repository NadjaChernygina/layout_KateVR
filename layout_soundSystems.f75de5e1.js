...
============================== */ (function initTestimonials(selector = '.testimonials') {
    ...
    function initOne(root) {
        ...
        if (!viewport || !track || !slides.length || !pager) return;
        // Autogenerate dots if empty
        if (!pager.children.length) {
            slides.forEach((s, i)=>{
                if (!s.id) s.id = `ts-slide-${i + 1}`;
            });
            pager.innerHTML = slides.map((s, i)=>`<button class="testimonials__dot" role="tab"
                 aria-selected="${i === 0}"
        ...
        function setIndex(i, moveFocus = false) {
            ...
            if (moveFocus) {
                const hasTransition = // eslint-disable-next-line no-undef
        ...
        viewport.addEventListener('touchend', ()=>{
            if (Math.abs(deltaX) > threshold) {
                if (deltaX > 0) setIndex(index - 1, true);
        ...
        dots.forEach((dot, i)=>{
            ...
            dot.addEventListener('keydown', (e)=>{
                if (e.key === 'ArrowRight') return setIndex(index + 1, true);
                if (e.key === 'ArrowLeft') return setIndex(index - 1, true);
                if (e.key === 'Home') return setIndex(0, true);
                if (e.key === 'End') return setIndex(slides.length - 1, true);
            });
        });
...
============================== */ (function initFeaturesSlider(rootSel = '.features', bpDesktop = '(min-width: 1024px)') {
    document.addEventListener('DOMContentLoaded', ()=>{
    ...
    function setup(root) {
        ...
        function renderFraction() {
            if (currentEl) currentEl.textContent = pad2(enabled ? index + 1 : 1);
        ...
        function setIndex(i, focusCard = false) {
            ...
            renderFraction();
            if (focusCard) {
                const card = slides[index].querySelector('.features__list-title') || slides[index];
                try {
                    card.focus?.({
                        preventScroll: true
                    });
        ...
        function bind() {
            ...
            viewport.addEventListener('touchstart', onTouchStart1, {
                passive: true
            });
            viewport.addEventListener('touchmove', onTouchMove1, {
                passive: true
            });
            viewport.addEventListener('touchend', onTouchEnd1);
            setIndex(0);
        }
        function unbind() {
            if (!enabled) return;
...
//# sourceMappingURL=layout_soundSystems.f75de5e1.js.map
